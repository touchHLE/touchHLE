...
+ (id)stringWithFormat:(id)format, // NSString*
    ...
    let text = from_rust_string(env, res);

    let new: id = msg![env; this alloc];
    let new: id = msg![env; new initWithString:text];
    autorelease(env, new)
}
...
- (id)initWithFormat:(id)format // NSString*
    ...
    init_with_format_into_host(env, this, format, args)
}
...
fn data_using_encoding(env: &mut Environment, this: id, encoding: NSStringEncoding) -> id {
    assert!(
