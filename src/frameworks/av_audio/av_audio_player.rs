...
- (id)initWithContentsOfURL:(id)url // NSURL*
    ...
    if !env.fs.is_file(GuestPath::new(&path_str)) {
        if !outError.is_null() {
            let domain = ns_string::from_rust_string(env, "NSOSStatusErrorDomain".to_owned());
            let error = msg_class![env; NSError alloc];
            let error = msg![env; error initWithDomain:domain code:2003334207 userInfo:nil];
    ...
    if status != 0 {
        if !outError.is_null() {
            let domain = ns_string::from_rust_string(env, "NSOSStatusErrorDomain".to_owned());
            let error = msg_class![env; NSError alloc];
            let error = msg![env; error initWithDomain:domain code:1954115647 userInfo:nil];
            env.mem.write(outError, error);
        }
        return nil;
    }
