...
impl Mem {
    ...
    pub fn vm_allocate(&mut self, size: GuestUSize) -> MutVoidPtr {
    ...
    /// Free an allocation made with `vm_allocate` or `reserve`. All allocations
    /// within the provided range are freed.
    pub fn vm_deallocate(&mut self, ptr: MutVoidPtr, size: GuestUSize) {
