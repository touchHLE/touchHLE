...
+ (())beginAnimations:(id)animation_id // NSString*
    ...
    if !context.is_null() {
        let context : id = msg_class![env; NSNumber numberWithUnsignedInt:(context.to_bits() as u32)];
...
+ (())commitAnimations {
    ...
    let animation_delegate = if delegate == nil {
        ...
        let total_animation_count = ca_transaction::State::get_current_transaction(env).unwrap().animations.len() as u32;
    ...
    for (layer, animation) in ca_transaction::State::get_current_transaction_mut(env).unwrap().animations.clone() {
    ...
    if ca_transaction::State::get_current_transaction(env).is_none() {
        for view in env.framework_state.uikit.ui_view.views.clone() {
            let layer: id = msg![env; view layer];
            ca_layer::set_use_implicit_animations(env, layer, false);
        }
...
- (())animationDidStop:(id)animation // CAAnimation*
    ...
    // TODO: Also notify when the animations are cancelled
