...
fn getenv(env: &mut Environment, name: ConstPtr<u8>) -> MutPtr<u8> {
    let name_cstr_vec = env.mem.cstr_at(name).to_vec();
    let name_cstr = name_cstr_vec.as_slice();
    ...
    let Some(value) = env.libc_state.stdlib.env.get(name_cstr).cloned() else {
        return match name_cstr {
            b"HOME" => {
                let ptr = env
                    .mem
                    .alloc_and_write_cstr(env.fs.home_directory().as_str().as_bytes());
                env.libc_state.stdlib.env.insert(name_cstr.to_vec(), ptr);
                ptr
            }
            _ => {
                log!(
