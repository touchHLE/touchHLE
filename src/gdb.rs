...
impl GdbServer {
    ...
    fn handle_monitor_command(
        ...
        fn strip_any_prefix<'a>(cmd: &'a str, prefixes: &[&str]) -> Option<&'a str> {
            for prefix in prefixes.iter() {
                if let Some(postfix) = cmd.strip_prefix(prefix) {
                    return Some(postfix);
                }
            }
            None
        ...
        if strip_any_prefix(cmd, &["h", "help"]).is_some() {
            let help_string =
...
htype/host_obj_type <ptr/reg>: Print the host type of the object at the pointer.";
        ...
        } else if strip_any_prefix(cmd, &["bt", "backtrace"]).is_some() {
        ...
        } else if let Some(ptr_str) = strip_any_prefix(cmd, &["htype", "host_obj_type"]) {
            let (ptr, _) = get_next_ptr_or_reg(&env.cpu, ptr_str)?;
            let obj = MutPtr::from_bits(ptr);
            env.objc
                .get_host_object(obj)
