...
impl Mem {
    ...
    pub fn alloc_in(&mut self, heap: Option<&mut HeapAllocator>, size: GuestUSize) -> MutVoidPtr {
    ...
    pub fn malloc_size(&mut self, ptr: ConstVoidPtr) -> GuestUSize {
        self.malloc_size_in(None, ptr)
    }

    pub fn malloc_size_in(
        &mut self,
    ...
    pub fn realloc(&mut self, old_ptr: MutVoidPtr, size: GuestUSize) -> MutVoidPtr {
        self.realloc_in(None, old_ptr, size)
    }
    ...
    pub fn realloc_in(
        ...
        if old_ptr.is_null() {
            return self.alloc_in(heap, size);
        }
        // TODO: for a moment we always assume that we do not have enough size
        //       to realloc inplace
        let old_size = self.malloc_size_in(heap.as_deref_mut(), old_ptr.cast_const());
        if old_size >= size {
            return old_ptr;
        }
        let new_ptr = self.alloc_in(heap.as_deref_mut(), size);
        self.memmove(new_ptr, old_ptr.cast_const(), old_size);
        self.free_in(heap, old_ptr);
        new_ptr
    }
    ...
    pub fn free(&mut self, ptr: MutVoidPtr) {
        self.free_in(None, ptr);
    }

    /// Free an allocation made with one of the `alloc` methods in `heap`.
    pub fn free_in(&mut self, heap: Option<&mut HeapAllocator>, ptr: MutVoidPtr) {
        let (vm, heap) = self.allocators_mut(heap);
