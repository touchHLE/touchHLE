...
impl Font {
    fn scale(&self, font_size: f32) -> Scale {
        ...
        Scale::uniform(font_size * if self.is_substitute { 1.125 } else { 1.0 })
    }
    ...
    fn calculate_line_width(&self, font_size: f32, line: &str) -> f32 {
        ...
        for glyph in self.font.layout(line, self.scale(font_size), Default::default()) {
