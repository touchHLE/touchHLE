...
/// NSCalendar identifier for the Gregorian calendar.
...
+ (id)currentCalendar {
    if let Some(calendar) = State::get(env).current_calendar {
        ...
        // TODO: Support other calendar types if any app needs them.
        env.objc.borrow_mut::<NSCalendarHostObject>(new_calendar)
            .calendar_identifier = get_static_str(env, NS_GREGORIAN_CALENDAR);
...
- (id)components:(NSCalendarUnit)unit_flags fromDate:(id)date { // NSDate *
    ...
    assert!(ident != nil);
    let greg: id = get_static_str(env, NS_GREGORIAN_CALENDAR);
