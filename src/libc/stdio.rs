...
impl State {
    ...
    fn get_file_host_obj_mut(env: &mut Environment, file_ptr: MutPtr<FILE>) -> &mut FILEHostObject {
        let FILE { fd } = env.mem.read(file_ptr);
        if matches!(fd, STDIN_FILENO | STDOUT_FILENO | STDERR_FILENO)
            && !env.libc_state.stdio.file_streams.contains_key(&file_ptr)
        {
            // Special case, need to do a lazy creation of host object
            env.libc_state.stdio.file_streams.insert(
                file_ptr,
                FILEHostObject {
                    pushbacks: Vec::new(),
                },
...
fn fgetc(env: &mut Environment, file_ptr: MutPtr<FILE>) -> i32 {
    ...
    let FILE { fd } = env.mem.read(file_ptr);
    let FILEHostObject { ref mut pushbacks } = State::get_file_host_obj_mut(env, file_ptr);
