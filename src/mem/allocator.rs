...
impl Chunk {
    pub fn new(base: VAddr, size: GuestUSize) -> Chunk {
        if size >= PAGE_SIZE.try_into().unwrap() {
...
mod collections {
    ...
    impl SizeBucketedChunkMap {
        ...
        fn allocate_in_bucket(&mut self, size: GuestUSize, bucket: usize) -> Option<Chunk> {
            ...
            if rump_size >= PAGE_SIZE.try_into().unwrap() && rump_base & 0xfff != 0 {
                assert!(rump_size > PAGE_SIZE.try_into().unwrap()); // TODO corner case
                assert!(existing.base & 0xfff == 0);
                // re-align base address by splitting in 2 chunks:
                // less than page size
                let left = Chunk::new(
                    existing.base + size,
                    GuestUSize::try_from(PAGE_SIZE).unwrap() - size,
                );
                // more than page size
                let right = Chunk::new(
                    existing.base + GuestUSize::try_from(PAGE_SIZE).unwrap(),
                    existing.size.get() - GuestUSize::try_from(PAGE_SIZE).unwrap(),
                );
                self.insert(left);
                self.insert(right);
            } else {
...
impl Allocator {
    ...
    pub fn free(&mut self, base: VAddr) -> GuestUSize {
            ...
            .or_else(|| self.unused_chunks.remove_with_end(freed.base))
        {
            ...
            if new_base >= PAGE_SIZE.try_into().unwrap() && new_size & 0xfff != 0 {
