...
fn fcntl(
    ...
    match cmd {
        ...
        F_GETLK => {
            let lock_ptr: MutPtr<FLockInfo> = args.start().next(env);
            ...
            // The simulator only returns an error when the lock begins before
            // start of the file if whence is set to `SEEK_SET`
            if whence == SEEK_SET && lock.start < 0 {
                set_errno(env, EINVAL);
                return -1;
            }
            ...
            // TODO: actually check set locks and return conflict if exists
        ...
        F_SETLK => {
            ...
            // upgraded, extended, unlocked, etc., but it will not conflict.
