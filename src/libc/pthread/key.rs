...
use crate::libc::errno::{EINVAL, EAGAIN};
...
fn pthread_key_create(
    ...
    if State::get(env).keys.len() < PTHREAD_KEYS_MAX as usize {
        ...
        State::get(env).keys.push(Some((HashMap::new(), destructor)));
        env.mem.write(key_ptr, key);
...
fn pthread_getspecific(env: &mut Environment, key: pthread_key_t) -> MutVoidPtr {
    let idx: usize = key.wrapping_sub(1).try_into().unwrap(); // wrap around to u32::MAX, keys.get will handle it
...
fn pthread_setspecific(env: &mut Environment, key: pthread_key_t, value: ConstVoidPtr) -> i32 {
    let idx: usize = key.wrapping_sub(1).try_into().unwrap(); // wrap around to u32::MAX, keys.get_mut will handle it
    let current_thread = env.current_thread;
