...
// Note that source files are looked for in the path "{tests_dir}/{test_app_name}_source"
// and binaries are output as "{tests_dir}/{test_app_name}.app/{test_app_name}".
// You also need crt1.c and the llvm/ path in tests_dir.
fn run_test_app(
    tests_dir: &Path,
    test_app_name: &str,
    sources: &[&Path],
...
#[test]
fn run_c_test_app() -> Result<(), Box<dyn Error>> {
    // We want to use lld by default for this test, since this test is expected to work on windows.
    let linker_path = env::var("TOUCHHLE_LINKER")
        .map_or("-fuse-ld=lld".to_owned(), |linker_path| {
    ...
    let sources = ["main.c"].map(|file| Path::new(file));
...
#[test]
#[ignore]
fn run_objc_test() -> Result<(), Box<dyn Error>> {
    // By default this uses the system linker, which is expected to ld for Mac OS.
    // By setting TOUCHHLE_LINKER to point to a ported (Apple) ld, you can also build on
    // linux (and potentially windows).
    ...
    let extra_compile_args = [
        ...
        // Unfortunately, it doesn't seem like it's possible to link libgcc_s in clang, which is
        // needed for exception handling.
        // TODO: Could we patch in the requirement afterwards?
        "-fno-objc-exceptions",
        "-ObjC",
    ];
