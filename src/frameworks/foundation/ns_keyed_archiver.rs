...
+ (id)allocWithZone:(NSZonePtr)_zone {
    ...
    plist.insert("$objects".into(), Value::Array(Vec::new()));
...
- (())encodeObject:(id)object /* NSCoding* */ forKey:(id)key /* NSString* */ {
    let key = normalize_key(env, key);
    encode_object_for_key(env, this, object, key);
}

- (())encodeBytes:(ConstPtr<u8>)bytes length:(NSUInteger)length forKey:(id)key /* NSString* */ {
    let key = normalize_key(env, key);
    let data = env.mem.bytes_at(bytes.cast(), length).to_vec();
    let scope = get_value_to_encode_for_key(env, this);
    assert_eq!(scope.contains_key(&key), false);
    scope.insert(key.into(), Value::Data(data));
}
...
fn normalize_key(env: &mut Environment, key: id) -> String {
    ...
    assert_eq!(key.starts_with('$'), false); // TODO: Mangle keys with $ prefix
    key.to_string()
}
...
fn encode_object_for_key(env: &mut Environment, this: id, object: id, normalized_key: String) {
    ...
    let uid = if let Some(existing_uid) = host_object.already_archived.get(&object).cloned() {
        ...
        if object == class {
            ...
            let mut superclass = env.objc.get_superclass(class);
            while superclass != nil {
                let superclass_name = env.objc.get_class_name(superclass);
                classes.push(Value::String(superclass_name.into()));
                superclass = env.objc.get_superclass(superclass);
            }
            let host_object = env.objc.borrow_mut::<NSKeyedArchiverHostObject>(this);
            ...
            let previous_key = std::mem::replace(
                &mut env
                    .objc
    ...
    let scope = get_value_to_encode_for_key(env, this);
    assert_eq!(scope.contains_key(&normalized_key), false);
    scope.insert(normalized_key.into(), Value::Uid(uid));
}
