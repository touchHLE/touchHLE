...
impl Environment {
    ...
    /// function returns to the main run loop ([Environment::run])
    pub fn wait_for_selector(&mut self, thread: ThreadId, sel: SEL, arg: id) {
        assert!(matches!(
            self.threads[self.current_thread].blocked_by,
            ThreadBlock::NotBlocked
        ));
        ...
        self.threads[self.current_thread].blocked_by =
            ThreadBlock::WaitingForSelector(thread, sel, arg);
    }
    ...
    pub fn unblock_wait_for_selector(&mut self, thread: ThreadId) {
        let ThreadBlock::WaitingForSelector(waiting_thread, sel, arg) =
            self.threads[thread].blocked_by
        else {
            panic!()
        };
        assert_eq!(waiting_thread, thread);
        log_dbg!(
