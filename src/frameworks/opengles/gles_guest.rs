...
use crate::gles::gles11_raw::types::GLintptr as HostGLintptr;
...
fn glMapBufferOES(env: &mut Environment, target: GLenum, access: GLenum) -> MutPtr<GLvoid> {
    //  "glMapBuffer maps to the client's address space the entire data store of the buffer
    ...
    //
    // We have to make an address space in the guest, monitor every read/write, and "forward"
    // those reads/writes to the address space in the host, which is mapped to the target buffer.
    // Since the mapped buffer can't be used until it's unmapped, we defer the "forwarding" of read/writes
    // until the moment the buffer is unmapped.
    assert!(!env
        .framework_state
        .opengles
        .mapped_buffers
    ...
    if host_buffer.is_null() {
        ...
        let buffer_size = _getBufferSize(env, target) as u32;
...
fn glUnmapBufferOES(env: &mut Environment, target: GLenum) -> GLboolean {
    ...
    //  buffer object's data store. When a data store is unmapped, the pointer to its data store becomes invalid."
    // https://docs.gl/gl2/glMapBuffer
    //
    // Since the mapped buffer can't be used until it's unmapped, we defer the "forwarding" of read/writes
    // until the moment the buffer is unmapped.
...
fn _getBufferSize(env: &mut Environment, target: GLenum) -> GLint {
    with_ctx_and_mem(env, |gles, _mem| {
