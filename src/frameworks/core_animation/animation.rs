    ...
    T: Clone,
{
    ...
    } else if from_value.is_some() && to_value.is_some() {
        let from_value = from_value.unwrap();
        let to_value = to_value.unwrap();
        let by_value = to_value - from_value;
        (from_value.to_owned(), by_value)
    } else if from_value.is_some() && by_value.is_some() {
        let from_value = from_value.unwrap();
        let by_value = by_value.unwrap();
        (from_value.to_owned(), by_value.to_owned())
    } else if to_value.is_some() && by_value.is_some() {
        let to_value = to_value.unwrap();
        let by_value = by_value.unwrap();
        let from_value = to_value - by_value;
        (from_value, by_value.to_owned())
    } else if from_value.is_some() {
        let from_value = from_value.unwrap();
        let by_value = current_value - from_value;
        (from_value.to_owned(), by_value)
    } else if to_value.is_some() {
        let from_value = current_value;
        let to_value = to_value.unwrap();
        let by_value = to_value - from_value;
        (from_value.to_owned(), by_value)
    } else if by_value.is_some() {
        let from_value = current_value;
        let by_value = by_value.unwrap();
        (from_value.to_owned(), by_value.to_owned())
    } else {
    ...
    for (key, animation) in presentation.animations.clone() {
        ...
        match &*key_path {
            ...
            "backgroundColor" => {
                ...
                let (from_value, by_value) = get_from_and_by_values(
                    &presentation.background_color.unwrap(),
                    from_value,
                    to_value,
                    by_value,
                );
            ...
            "hidden" => {
                ...
                let by_value = id_as_option!(by_value)
                    ...
                    .map(|val: bool| val as i32 as f32);
                let (from_value, by_value) = get_from_and_by_values(
                    &(presentation.hidden as i32 as f32),
                    from_value.as_ref(),
                    to_value.as_ref(),
                    by_value.as_ref(),
                );
