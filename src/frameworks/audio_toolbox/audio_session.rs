...
fn AudioSessionSetProperty(
    ...
    match in_ID {
        kAudioSessionProperty_AudioCategory => {
            let value: u32 = env.mem.read(in_data.cast());
            state.audio_session_category = value;
        }
        kAudioSessionProperty_PreferredHardwareIOBufferDuration => {
            log!("TODO: AudioSessionSetProperty(kAudioSessionProperty_PreferredHardwareIOBufferDuration, {:?}, {:?})", in_data_size, in_data);
        }
        _ => unreachable!(),
    }
