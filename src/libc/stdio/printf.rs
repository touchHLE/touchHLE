...
pub fn printf_inner<const NS_LOG: bool, F: Fn(&Mem, GuestUSize) -> u8>(
    ...
    loop {
        ...
        let length_modifier = match get_format_char(&env.mem, format_char_idx) {
            ...
            // q seems to be an equivalent of 'll' - https://developer.apple.com/library/archive/documentation/Cocoa/Conceptual/Strings/Articles/formatSpecifiers.html#//apple_ref/doc/uid/TP40004265-SW1
            b'q' => {
                format_char_idx += 1;
                Some("ll")
            }
