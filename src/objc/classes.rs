...
fn substitute_classes(
    ...
    match bundle.bundle_identifier() {
        "com.chillingo.defenderchronicles" => {
            // could probably ignore all OF starting classes to make sure
            // but to avoid issues this keeps the list to a minimum
            if !(name.starts_with("OFHighScoreService") || name.starts_with("OpenFeint")) {
                return None;
            }
        }
        _ => {
            // Currently the only thing we try to substitute: classes that
            // seem to be from various third-party advertising or social network
            // SDKs. Naturally it makes a lot of use of UIKit and networking in
            // ways we don't support yet. This isn't "ad blocking" because ads
            // no longer work on real devices anyway :)
