            ...
            match p.as_bytes()[0] {
                ...
                b'v' => {
                    if p == "vCont?" {
                        ...
                        for (thread, curr_state) in env.threads.iter_mut().zip(states.into_iter()) {
                            if !thread.is_alive() {
                                continue;
                            }
                            match curr_state {
                ...
                b'q' => {
                    if p == "qC" {
                        ...
                        self.send_packet(format!("QC{:x}", tid).as_str());
                        ...
                        thread_block
                            .as_bytes()
                            .iter()
                            .for_each(|b| write!(thread_block_hex, "{:02x}", b).unwrap());
