...
use touchHLE_openal_soft_wrapper::{al_types::{ALvoid, ALuint}, alSourcePlay, alDeleteBuffers, alDeleteSources, alGenBuffers, alBufferData, AL_FORMAT_STEREO16, alSourcei, alGenSources, AL_BUFFER, al_types::ALint, AL_LOOPING, alSourcef, AL_GAIN, alSourceStop, AL_FORMAT_MONO8, AL_FORMAT_MONO16, AL_FORMAT_STEREO8};

use crate::{
    objc::{id, ClassExports, HostObject, nil, release, retain},
    objc_classes, frameworks::foundation::{NSInteger, NSTimeInterval, ns_url::to_rust_path}, audio::{AudioFile, AudioDescription}, msg, environment::Environment,
};
use std::{alloc::{alloc, dealloc, Layout}, mem::align_of};
...
- (bool)prepareToPlay {
    ...
    let audio_format = {
        if channels_per_frame == 1 && bits_per_channel == 8 {
            ...
            // TODO: 0 bits_per_channel means the audio file is compressed, audio_file.read_bytes must decompress it.
...
- (bool)play {
    ...
    // https://developer.apple.com/documentation/avfaudio/avaudioplayer/1387388-play?language=objc
...
- (())setNumberOfLoops:(NSInteger)numberOfLoops {
    ...
    if let Some((_, source, _, _)) = host_object.audio_data {
        if numberOfLoops < 0 {
            unsafe { alSourcei(source, AL_LOOPING, 1); }
        } else if numberOfLoops == 0 {
            unsafe { alSourcei(source, AL_LOOPING, 0); }
        } else {
            unimplemented!()
        }
    }
