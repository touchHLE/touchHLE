...
fn fcntl(
    ...
    match cmd {
        ...
        F_SETFD => {
            // SET/GETFD are responsible for managing the the CLOEXEC flag on an
            // FD. When set, exec* calls automatically close open FDs inorder to
            // prevent them fromo leaking. Since touchHLE only runs a single
            // process and non-jailbroken apps can not call exec* functions. 
            // It's safe to ignore the flag beingn set.
