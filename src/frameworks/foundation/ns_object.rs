...
forUndefinedKey:(id)key { // NSString*
    ...
    panic!("Object {:?} of class {:?} ({:?}) does not have a setter for {} ({:?})\nAvailable selectors: {}\nAvailable ivars: {}", this, class_name_string, class, key_string, key, env.objc.debug_all_class_selectors_as_strings(&env.mem, class).join(", "), env.objc.debug_all_class_ivars_as_strings(class).join(", "));
}
