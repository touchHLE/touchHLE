...
fn CFRunLoopTimerCreate(
    ...
    let fake_target: id = msg_class![env; _CFTimerTarget alloc];
...
/// _CFTimerTarget is not a real class in the sense that it
/// is not a part of Apple's frameworks, but rather serves
/// as a convenience object for performing a callout from
/// a timer
struct _CFTimerTargetHostObject {
    callout: GuestFunction,
    info: MutVoidPtr,
}
impl HostObject for _CFTimerTargetHostObject {}

pub const CLASSES: ClassExports = objc_classes! {

(env, this, _cmd);

@implementation _CFTimerTarget: NSObject

+ (id)allocWithZone:(NSZonePtr)_zone {
    let host_object = Box::new(_CFTimerTargetHostObject {
...
- (id)initWithCallout:(GuestFunction)callout info:(MutVoidPtr)info {
    let host_object: &mut _CFTimerTargetHostObject = env.objc.borrow_mut(this);
