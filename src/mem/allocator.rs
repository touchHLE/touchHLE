...
impl Chunk {
    ...
    #[inline(always)]
    fn includes(&self, other: Chunk) -> bool {
        self.contains(other.base) && self.contains(other.last_byte())
    }
...
impl VMAllocator {
    ...
    fn allocate_at(&mut self, address: VAddr, size: GuestUSize) -> Option<Chunk> {
        ...
        let to_carve = self
            .unused_chunks
            .iter()
            .find(|unused_chunk| unused_chunk.includes(chunk))?;

        let (before, after) = to_carve.difference(chunk);
        self.unused_chunks.remove_with_base(to_carve.base);
