...
use crate::objc::{msg, msg_class, id};
...
fn CFBundleCopyBundleLocalizations(env: &mut Environment, bundle: CFBundleRef) -> CFArrayRef {
    let bundleLocalizations = env.objc.borrow_mut::<NSBundleHostObject>(bundle)._bundle.as_ref()
        .unwrap_or(&env.bundle)
        .bundle_localizations().to_owned();
    ...
    log_dbg!("CFBundleCopyBundleLocalizations({:?}) => {:?} ({})", bundle, locArray, localizationStrings.join(", "));
    locArray
}

fn CFBundleCopyPreferredLocalizationsFromArray(_env: &mut Environment, locArray: CFArrayRef) -> CFArrayRef {
    ...
    log!("TODO: CFBundleCopyPreferredLocalizationsFromArray({:?}) => {:?}", preferredLocalizations, preferredLocalizations);
