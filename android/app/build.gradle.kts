...
fun runTouchHLEVersionTool(wantBranding: Boolean): String {
    val command = mutableListOf("cargo", "run", "--package", "touchHLE_version")
    if (wantBranding) {
        command.addAll(listOf("--", "--branding"))
    }

    return ProcessBuilder(command).start().inputStream.bufferedReader().readText().trim()
}
...
cargoNdk {
    ...
    if (DefaultNativePlatform.host().operatingSystem.isWindows) {
        val binPath = android.ndkDirectory.toPath().resolve("toolchains/llvm/prebuilt/windows-x86_64/bin")
    ...
    extraCargoBuildArguments = arrayListOf("--lib", "--no-default-features", "--features", "touchHLE_openal_soft_wrapper/static,sdl2/bundled")
}
