...
use super::cg_context::{
    CGContextHostObject, CGContextRef, CGContextSubclass, CGBlendMode,
    kCGBlendModeNormal, kCGBlendModeClear, kCGBlendModeCopy, kCGBlendModeSourceIn,
    kCGBlendModeSourceOut, kCGBlendModeSourceAtop, kCGBlendModeDestinationOver,
    kCGBlendModeDestinationIn, kCGBlendModeDestinationOut, kCGBlendModeDestinationAtop,
    kCGBlendModeXOR, kCGBlendModePlusLighter,
};
use super::cg_image::{
...
fn put_pixel(
    ...
    let res = match blend_mode {
        ...
        kCGBlendModeNormal | _ => {
            // Normal is SourceOver
            (
                fg.0 + bg.0 * (1.0 - fg.3),
                fg.1 + bg.1 * (1.0 - fg.3),
                fg.2 + bg.2 * (1.0 - fg.3),
