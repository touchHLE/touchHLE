...
fn glMapBufferOES(env: &mut Environment, target: GLenum, access: GLenum) -> MutPtr<GLvoid> {
    ...
    assert!(!env.framework_state.opengles.mapped_buffers.contains_key(&(
        env.current_thread,
        target,
        buffer_object_name
    )));
    if host_buffer.is_null() {
        ...
        env.framework_state.opengles.mapped_buffers.insert(
            (env.current_thread, target, buffer_object_name),
            (guest_buffer, host_buffer),
        );
        guest_buffer
    }
}
fn glUnmapBufferOES(env: &mut Environment, target: GLenum) -> GLboolean {
    ...
    let buffer_object_name = _get_currently_bound_buffer_object_name(env, target);
    if let Some((guest_buffer, host_buffer)) = env
        .framework_state
        .opengles
        .mapped_buffers
        .remove(&(env.current_thread, target, buffer_object_name))
    {
        let buffer_size = _get_buffer_size(env, target) as u32;
