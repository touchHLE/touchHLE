...
pub fn AudioFileOpenWithCallbacks(
    ...
    if size == 0 {
        panic!("0 byte size of file for AudioFileOpenWithCallbacks(), likely bad!");
    }
    ...
    if env.mem.read(bytes_read_ptr) != size {
        panic!("Bytes read != size for AudioFileOpenWithCallbacks(), likely bad!");
    }
