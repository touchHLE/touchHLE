...
use crate::window::{GLContext, GLVersion, GLYieldFreeSection, Window};
...
impl GLESContext for GLES1OnGL2Context {
    ...
    fn make_current<'a>(&'a self, window: &Window) -> Box<dyn GLES + 'a> {
        if self.gl_ctx.is_current() && self.is_loaded.load(std::sync::atomic::Ordering::Relaxed) {
            return Box::new(GLES1OnGL2 {
                yield_free_section: window.make_yield_free_section(),
                state: &self.state,
            });
        }
        ...
        Box::new(GLES1OnGL2 {
            yield_free_section: critical_section,
            state: &self.state,
        })
    }
}
...
pub struct GLES1OnGL2<'a> {
    ...
    #[allow(unused)]
    yield_free_section: GLYieldFreeSection,
}
