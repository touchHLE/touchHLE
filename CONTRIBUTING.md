...
You should also run `cargo test`. [Building the integration tests requires downloading LLVM](tests/BUILDING.md), so it's understandable if you want to skip them (`cargo test -- --skip run_c_test_app`) and let the GitHub Actions CI catch any issues when you submit your pull request. Alternatively, you can download a pre-built versions of the integration tests apps (TestApp/ObjCTestApp) from GitHub Actions CI and run it in touchHLE.
