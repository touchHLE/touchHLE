...
impl Bundle {
    ...
    fn icon_path(&self) -> GuestPathBuf {
        ...
        // (Using CFBundleIconsFiles case-insensitive) *Icon.png, *Icon-72.png,
        // First in CFBundleIconFiles (adds .png if not present),
        // First in CFBundleIconFile (adds .png if not present),
        // Failsafe Icon.png
        if let Some(filename) = self
            ...
            .and_then(|a| {
                ["icon.png", "icon-72.png"]
                    ...
                    .find_map(|icon| {
                        a.iter().find(|v| {
                            v.as_string().is_some_and(|s| s.to_lowercase().ends_with(*icon))
                        })
                    })
                    .or_else(|| a.first())
            })
