...
fn topological_sort(graph: &[Node]) -> Result<Vec<usize>, String> {
...
mod dylib_sorting_tests {
    ...
    fn create_dylib_graph(bin_configs: &[(&str, &[&str])]) -> Vec<Node> {
        bin_configs
            .iter()
            .map(|(name, dependencies)| Node {
                name: name.to_string(),
    ...
    /// before their import
    fn verify_sort(graph: &[Node], sorted_indices: &[usize]) {
    ...
    fn test_no_dependencies() {
        ...
        let sorted_indices = topological_sort(&dylib_graph).unwrap();
    ...
    fn test_cycle() {
        ...
        let result = topological_sort(&dylib_graph);
