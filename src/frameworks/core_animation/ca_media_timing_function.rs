...
pub(super) struct State {
    named_functions: HashMap<String, id>,
}
...
+ (id)functionWithName:(CAMediaTimingFunctionName)name {
    ...
    let object = if let Some(object) = object {
        ...
        env.framework_state.core_animation.ca_media_timing_function.named_functions.insert(name_string.to_string(), object);
        retain(env, object)
    };
