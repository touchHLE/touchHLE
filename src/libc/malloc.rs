...
impl malloc_zone_t {
    pub fn new(env: &mut Environment) -> malloc_zone_t {
        malloc_zone_t {
            reserved1: Ptr::null(),
            reserved2: Ptr::null(),
            size: get_function_address(env, "malloc_zone_size"),
            malloc: get_function_address(env, "malloc_zone_malloc"),
            calloc: Ptr::null(),
            valloc: Ptr::null(),
            free: get_function_address(env, "malloc_zone_free"),
            realloc: get_function_address(env, "malloc_zone_realloc"),
            destroy: get_function_address(env, "malloc_destroy_zone"),
            zone_name: Ptr::null(),
...
/// Call a function using the heap corresponding to `zone`
fn with_zone<R>(
    env: &mut Environment,
    zone: MutPtr<malloc_zone_t>,
