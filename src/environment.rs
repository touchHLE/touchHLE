...
pub struct Environment {
    ...
    // proc macro magic.
    // TODO? Try to remove uses of "split borrows" so that the runtime can swap
    // this
    //
...
/// [Environment::run_with_yielder] for why this is needed.
...
impl Environment {
    ...
    pub fn new(
        ...
        // Enforces the one (real) Environment limit. See `run_with_yielder` for
        // why this is needed.
    ...
    pub fn new_without_app(
        ...
        // Enforces a one (real) Environment limit. See `run_with_yielder` for
        // why this is needed.
        if ENVIRONMENT_INSTANCE_EXISTS.load(std::sync::atomic::Ordering::Relaxed) {
            return Err("Only one (real) Environment can exist at a time!".to_string());
        }
    ...
    /// Environment in the passed function.
    ///
    ...
    /// condition is met.
    ///
    /// Note that this is blocked inside a "yield-free section". This is for
    /// when a stateful global resource (currently just OpenAL and OpenGL) is
    /// used. This is handled automatically for you by calling
    /// [crate::gles::GLESContext::make_current] or (BEFOREMERGE: There might be
    /// something similar to the current GLES context management for OpenAL,
    /// right now this is attached to
    /// [crate::frameworks::audio_toolbox::ContextManager])
    ///
    /// Inside functions that use these resources, you should not call
    /// any functions that may yield - significantly, this includes any guest
    /// code, as guest code yield at any time!
    ///
    /// If you do need to call a potentially yielding function, drop the active
    /// context manager, and reacquire it after the potentially yielding
    /// function.
    pub fn yield_thread(&mut self, thread_block: ThreadBlock) {
        assert!(!self.threads[self.current_thread].is_blocked());
        if self.in_yield_free_section() {
    ...
    /// Check the inner state objects to see if we are in a yield-free section.
    /// See [Self::yield_thread] for why this is needed.
    fn in_yield_free_section(&self) -> bool {
        self.window
            .as_ref()
            .is_some_and(|w| w.in_yield_free_section())
            || self.framework_state.in_yield_free_section()
    }
}
