...
impl Environment {
    ...
    fn stack_trace_all(&self) {
        echo_no_panic!("Attempting to produce stack trace for current thread (#{}):", self.current_thread);
        self.stack_trace_for_thread(self.current_thread);
    ...
    fn stack_trace_for_thread(&self, tid: usize) {
        if tid >= self.threads.len() {
            echo_no_panic!("Thread {} is too large ({} threads exist)!", tid, self.threads.len());
        }
    ...
    /// [crate::gles::GLESContext::make_current] or
    /// [crate::audio::openal::OpenALContext::make_current]
    ///
