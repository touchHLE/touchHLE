...
impl Mem {
    ...
    /// This is arbitrarily set to 256 MB, eventually the heap should grow.
    ...
    /// TODO: This is currently set based on the docs saying "a few
    /// pages", not on actual behavior.
    ...
    /// Free an allocation made with `alloc_paged` or `reserve`. This matches
    /// the behavior of vm_deallocate where all allocations within the provided
    /// range are freed and partial frees are supported.
    pub fn free_paged(&mut self, ptr: MutVoidPtr, size: GuestUSize) {
