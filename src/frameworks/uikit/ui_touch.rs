...
fn handle_touches_down(env: &mut Environment, map: HashMap<FingerId, Coords>) {
    ...
    for i in 0..touches_count {
        ...
        let touches_count: NSUInteger = msg![env; touches count];
        if !is_multi_touch_enabled && touches_count > 0 {
            log!("TODO: ignoring multi-touch {:?} touch for view {:?}, isMultipleTouchEnabled: false", touch, view);
            continue;
        }
