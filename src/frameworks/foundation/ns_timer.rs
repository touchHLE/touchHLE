...
struct NSTimerHostObject {
    ...
    /// If the timer is currently running, so that we don't run it while it is
    /// already running.
    is_running: bool,
    /// Weak reference
    run_loop: id,
...
pub(super) fn handle_timer(env: &mut Environment, timer: id) -> Option<Instant> {
    ...
    // If a timer is already running, we don't want to run it since it could
    ...
    env.objc.borrow_mut::<NSTimerHostObject>(timer).is_running = true;
    ...
    env.objc.borrow_mut::<NSTimerHostObject>(timer).is_running = false;
