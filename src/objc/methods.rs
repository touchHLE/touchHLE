...
use crate::mem::{guest_size_of, ConstPtr, GuestUSize, Mem, Ptr, SafeRead, MutVoidPtr};
...
impl ClassHostObject {
    ...
    pub(super) fn add_ivars_from_bin(
        ...
        for i in 0..count {
            let ivar_ptr: ConstPtr<ivar_t> =
                Ptr::from_bits(ivars_base_ptr.to_bits() + i * entsize);
...
impl ObjC {
    ...
    pub fn all_class_selectors_as_strings(&self, mem: &Mem, class: Class) -> Vec<String> {
        ...
        loop {
            ...
            let mut class_selector_strings: Vec<String> = methods.keys().into_iter().chain(ivars.keys().into_iter()).map(|sel| sel.as_str(mem).to_string()).collect();
