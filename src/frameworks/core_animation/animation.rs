...
pub fn create_presentation_layer(env: &mut Environment, layer: id) -> CALayerHostObject {
    ...
    for (key, animation) in presentation.animations.clone() {
        ...
        // TODO: Setting [repeatCount] to greatestFiniteMagnitude will cause the animation to repeat forever.
        if (repeat_count == 0.0 && progress >= 1.0) || current_repeat > repeat_count {
