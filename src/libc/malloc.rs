...
impl malloc_zone_t {
    pub fn new(env: &mut Environment) -> malloc_zone_t {
        malloc_zone_t {
            ...
            size: env
                .dyld
                .get_function_address(&mut env.mem, &mut env.cpu, "malloc_zone_size")
                .unwrap(),
            malloc: env
                .dyld
                .get_function_address(&mut env.mem, &mut env.cpu, "malloc_zone_malloc")
                .unwrap(),
            calloc: Ptr::null(),
            valloc: Ptr::null(),
            free: env
                .dyld
                .get_function_address(&mut env.mem, &mut env.cpu, "malloc_zone_free")
                .unwrap(),
            realloc: env
                .dyld
                .get_function_address(&mut env.mem, &mut env.cpu, "malloc_zone_realloc")
                .unwrap(),
            destroy: env
                .dyld
                .get_function_address(&mut env.mem, &mut env.cpu, "malloc_destroy_zone")
                .unwrap(),
            zone_name: Ptr::null(),
...
fn malloc_zone_free(env: &mut Environment, zone: MutPtr<malloc_zone_t>, ptr: MutVoidPtr) {
    with_zone(env, zone, |mem, heap| mem.free_in(heap, ptr))
}
...
fn malloc_zone_malloc(
    ...
    with_zone(env, zone, |mem, heap| mem.alloc_in(heap, size))
}
...
fn malloc_zone_realloc(
    ...
    with_zone(env, zone, |mem, heap| mem.realloc_in(heap, ptr, size))
}
...
fn malloc_zone_size(
    ...
    with_zone(env, zone, |mem, heap| mem.malloc_size_in(heap, ptr))
}
