...
pub fn create_presentation_layer(env: &mut Environment, layer: id) -> CALayerHostObject {
    ...
    let named_animations: Vec<(bool, usize, String, id)> = presentation.animations.iter().enumerate().map(|(idx, (key, anim))| (true, idx, key.clone(), *anim)).collect();
    let anonymous_animations: Vec<(bool, usize, String, id)> = presentation.anonymous_animations.iter().enumerate().map(|(idx, anim)| (false, idx, format!("anonymous #{}", idx), *anim)).collect();
    let mut anonymous_animations_to_remove: Vec<usize> = Vec::new();
    for (is_named_animation, idx, key, animation) in Iterator::chain(named_animations.iter(), anonymous_animations.iter()) {
        ...
        if current_repeat >= effective_repeat_count {
            ...
            if is_named_animation {
                ...
                anonymous_animations_to_remove.push(idx);
            }
            continue;
        }
    ...
    for index in anonymous_animations_to_remove.iter().rev() {
        // Remove in reverse order to avoid removing the wrong animations
        remove_anonymous_animation_at_index(env, layer, *index);
    }
