...
- (id)stringByExpandingTildeInPath {
    // TODO: Deal with edge cases like ~foo/bar and/or ~user
    // These get mapped (in sandbox folder) to app/foo/bar or app/user
    // That is obviously wrong, but for the sake of touchHLE should work
    // Have yet to see an iOS app that uses this function to expand ~user
    let path = to_rust_string(env, this);

    let new_path_str = if path.starts_with('~') {
        let path = path.trim_start_matches('~');
        // TODO: Fix this?
        // Unix paths should only have ~ as first char
