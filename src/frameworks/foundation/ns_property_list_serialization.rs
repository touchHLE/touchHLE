...
use crate::objc::{ClassExports, id, msg, msg_class, nil, release};
...
fn serialize_plist(env: &mut Environment, obj: id) -> Value {
    ...
    if msg![env; obj isKindOfClass: dict_class] {
        ...
        match env.mem.read(type_str) {
            b'B' => {
                Value::Boolean(msg![env; obj boolValue])
            },
            b'Q' => {
            ...
            b'd' => {
                Value::Real(msg![env; obj doubleValue])
            },
        ...
        Value::Array(to_vec(env, obj).iter().map(|&x| serialize_plist(env, x)).collect())
    } else {
        todo!("Serializing {} not supported yet", env.objc.get_class_name(class))
    }
}
