...
pub fn pread(
    ...
    // Rust doesn't provide a way of reading at a specific offset without
    // affecting the underlying files cursor. Rather that bringing in a library
    // that does this or dealing with the unsafe windows API directly
...
pub fn pwrite(
    ...
    // Rust doesn't provide a way of writing at a specific offset without
    // affecting the underlying files cursor. Rather that bringing in a library
    // that does this or dealing with the unsafe windows API directly
    // (ReadFile + Overlapped), we can emulate the behavior with a set of seek
    // and write calls.
