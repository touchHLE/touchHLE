...
+ (())setValue:(id)value forKey:(id)key {
    ...
    log_once!("TODO: [CATransaction setValue:{:?} forKey:{:?}]", value, key);
}
...
+ (())setDisableActions:(bool)flag {
    // very spammy, so using log_once
    log_once!(" TODO: [CATransaction setDisableActions:{:?}]", flag);
}

+ (())setAnimationDuration:(CFTimeInterval)duration {
    // very spammy, so using log_once
    log_once!("TODO: [CATransaction setAnimationDuration:{:?}]", duration);
}
