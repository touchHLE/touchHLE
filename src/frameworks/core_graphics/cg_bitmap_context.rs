...
use super::cg_context::{
    ...
    kCGBlendModeMultiply, kCGBlendModeNormal, kCGBlendModeScreen, kCGBlendModeSourceAtop,
    kCGBlendModeSourceIn, kCGBlendModeSourceOut, CGBlendMode, CGContextHostObject, CGContextRef,
    CGContextSubclass,
};
use super::cg_image::{
...
fn blend_premultiplied(
    ...
    let (blend_res, keep_fg_out, keep_bg_out, new_a) = match blend_mode {
        ...
        kCGBlendModeSourceAtop => ((bg.3 * fg.0, bg.3 * fg.1, bg.3 * fg.2), false, true, bg.3),
        _ => unimplemented!(
            "blend mode {} / {}",
            blend_mode,
