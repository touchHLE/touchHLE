...
/// Helper for creating a yield-free section that doesn't allow thread yields
/// or creation of other GLES instances inside it. This only needs to be used
/// by GLES management code.
pub struct GLYieldFreeSection {
    is_active: Rc<AtomicBool>,
}

impl GLYieldFreeSection {
    pub fn new(is_active: Rc<AtomicBool>) -> Self {
        assert!(!is_active.load(std::sync::atomic::Ordering::Relaxed));
        is_active.store(true, std::sync::atomic::Ordering::Relaxed);
        GLYieldFreeSection { is_active }
    }
}

impl Drop for GLYieldFreeSection {
    fn drop(&mut self) {
        self.is_active
...
pub struct Window {
    ...
    /// Ensures that only one OpenGL instance is "active" at a time.
...
impl Window {
    ...
    #[must_use]
    pub unsafe fn make_gl_context_current(&self, gl_ctx: &GLContext) -> GLYieldFreeSection {
        self.window.gl_make_current(&gl_ctx.0).unwrap();
        GLYieldFreeSection::new(self.gl_instance_active.clone())
    }
    ...
    #[must_use]
    pub fn make_yield_free_section(&self) -> GLYieldFreeSection {
        GLYieldFreeSection::new(self.gl_instance_active.clone())
    }
