...
bitflags! {
    ...
    pub struct VmFlags : i32 {
        const VM_FLAGS_ANYWHERE = 0x1;
        const VM_FLAGS_PURGABLE = 0x2;
    }
}
...
pub fn vm_allocate(
    ...
    const HANDLED_FLAGS: VmFlags = VmFlags::VM_FLAGS_ANYWHERE;

    let tag = (flags >> 24) as u8;
    let flags = VmFlags::from_bits_retain(flags & !VM_TAG_MASK);
    ...
    let address = (!flags.contains(VmFlags::VM_FLAGS_ANYWHERE)).then(|| env.mem.read(address_ptr));
