...
- (id)initWithFrame:(CGRect)frame {
    ...
    let selector = env.objc.lookup_selector("_touchHLE_flip").unwrap();
...
- (())_touchHLE_flip {
    ...
    // It's not clear which event should be passed here.
    // On iOS Simulator, it is always nil.
    // And on real device, it's an event with empty touches.
